...
@bp.route('/api/businesses/<int:business_id>/subscription/cancel', methods=['POST'])
@login_required
@system_admin_api_required
def cancel_subscription(business_id):
    ...
    try:
        ...
        if immediate:
            ...
            business.subscription_status = 'pending_cancellation'
            business.updated_at = datetime.now(timezone.utc)
        ...
        from app.auth import log_audit
        log_audit('cancel_subscription', 'business', business_id, {
            'immediate': immediate,
            'reason': reason,
            'business_name': business.business_name
        })
